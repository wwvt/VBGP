...
hypsk.mean = hypgp.mean; 
hypsk.cov = hypgp.cov; 
hypsk = minimize(hypsk, @gp_ww, -100, @infGaussLik_ww, meanfunc, covfunc, [], X, Y_init(:,Mreps),Vhat_init(:,Mreps)./n_init); 

Xpred5k = (minx:((maxx-minx)/(5000-1)):maxx)';              % for calculating W
Ks = feval('covSEard', hypsk.cov, X, Xpred5k); 
SigmaM = feval('covSEard', hypsk.cov, X, X);        % WW edited
...
hyp3(:,Mreps) = minimize(hyp3(:,Mreps), @gp_ww, -100, @infGaussLik_ww, meanfunc, covfuncsk, [], X, Y(:,Mreps),Vhat(:,Mreps)./n(:,Mreps)); 
[ymu, ys2, fmu, fs2, lp, post] = gp_ww(hyp3(:,Mreps), @infGaussLik_ww, meanfunc, covfuncsk, [], X, Y(:,Mreps),Vhat(:,Mreps)./n(:,Mreps),Xpred);
sky(:,Mreps) = fmu;
%---------------------SK predict---------------------%
...
save('VBGP_rev1_MM1_SK_SKORapproxBA_rule3_SK_C500k7_M115.mat');
