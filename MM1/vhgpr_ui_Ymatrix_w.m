function [NMSE, Ey, Vmean, mutst, diagSigmatst, atst, diagCtst, ... 
    LambdaTheta, loghyperGP, convergence] = vhgpr_ui_Ymatrix_w(x_tr, y_tr, x_tst, y_tst, A, iter, loghyperGP, loghyper)
% WW edited.  9/22/2017
...
% for dataset with Ymatrix instead of the y_tr(l).n structure % WW added
Y = mean(y_tr,2);
...
%vhgpr_w(X=LambdaTheta, covfunc1, covfunc2, fixhyp, X, y, A, Xtst)
   [LambdaTheta, convergence0] = minimize(LambdaTheta, 'vhgpr_w_Ymatrix_1101', 30, covfuncSignal, covfuncNoise, 2, x_tr, y_tr, A); % A added % error here % 2=fixhyp
  
  % LambdaTheta = fminsearch('vhgpr_w_Ymatrix_1101_derivfree',LambdaTheta, 30, covfuncSignal, covfuncNoise, 2, x_tr, y_tr, A ) ;
  % fminsearch changes LabdaTheta but leads to bad MSE
  display('Running VHGPR...')
 %   [LambdaTheta, convergence] = minimize(LambdaTheta, 'vhgpr_w_Ymatrix_1101', iter, covfuncSignal, covfuncNoise, 0, x_tr, y_tr, A); % A added
     [LambdaTheta, convergence] = minimize(LambdaTheta, 'vhgpr_w_Ymatrix_1101', iter, covfuncSignal, covfuncNoise, 0, x_tr, y_tr, A); % A added
     convergence = [convergence0; convergence];
else % Use given, fixed hyperparameters
    display('Running VHGPR...')
    LambdaTheta = [log(0.5)*ones(n,1);loghyper];
    [LambdaTheta, convergence] = minimize(LambdaTheta, 'vhgpr_w_Ymatrix_1101', iter, covfuncSignal, covfuncNoise, 2, x_tr, y_tr, A); % A added
end

%Prediction
if nargout > 1
    [Ey, Vmean, mutst, diagSigmatst, atst, diagCtst]= vhgpr_w_Ymatrix_1101(LambdaTheta, covfuncSignal, covfuncNoise, 0, x_tr, y_tr, A, x_tst); % A added
