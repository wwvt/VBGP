function [varargout] = sk_ww_SKORapprox(hyp, inf, mean, cov, lik, x, y, Vhat, xs, ys)
...
%     end
%   end
  if nargin>8   % compute marginal likelihood and its derivatives only if needed
    if isstruct(y)
      post = y;            % reuse a previously computed posterior approximation
    else
      post = feval(inf{:}, hyp, mean, cov, x, y, Vhat);  % get the posterior here WW edited
    end
   else
    if nargout<=1
 %     [post nlZ] = feval(inf{:}, hyp, mean, cov, lik, x, y, Vhat); dnlZ = {}; %WW edited - actually not executed
    else
      [post nlZ dnlZ] = feval(inf{:}, hyp, mean, cov, x, y , Vhat); %WW edited - executed for training hyp
    end
  end
...
if nargin==8                                     % if no test cases are provided
  ...
  % tril:returns the lower triangular part of X. % edited
  ...
  while nact<ns               % process minibatches of test cases to save memory
    ...
    SigmaM = feval(cov{:}, hyp.cov, x(nz,:), x(nz,:));        % WW edited
...
%     fs2(id) = kss - reshape(diag(Ks'*inv(L)'* inv(L)*Ks)',length(id),1);                       % predictive variances
      V  = L\Ks;
      fs2(id) = kss - sum(V.*V,1)';                       % predictive variances %need to be modified 10/10/2017
    ...
    ys2(id) = sum(reshape(Fs2(:),[],N),2)/N;                          % .. variance
    nact = id(end);          % set counter to index of last processed data point
  end
  if nargin<9
  else
    varargout = {ymu, ys2, fmu, fs2, lp, post, Ks, SigmaM};
  end
end
