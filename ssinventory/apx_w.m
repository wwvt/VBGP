function K = apx_w(hyp,cov,x,opt)
% Last update: 10/10/2017. The code was modified to be
 %   L = chol(WDiag+K,'lower');             % Cholesky factor of B
 %    solveKiW = @(r) solve_chol(L',r);   original (L,r) where L means upper
 %  dhyp.cov = dK( inv(L)'* inv(L) - R )/2;
...
function [ldB2,solveKiW,dldB2,L] = ldB2_exact_w(W,K,dK)
...
%   
function dhyp = ldB2_deriv_exact_w(W,dK,L, alpha)
  if nargin>3, R = alpha*alpha'; else R = 0; end
 % if nargin>5, R = R + 2*a*b'; end
 % dhyp.cov = dK( bsxfun(@times,Q,W) - R )/2;
 dhyp.cov = dK( inv(L)'* inv(L) - R )/2;
