function [post nlZ dnlZ] = infMCMC(hyp, mean, cov, lik, x, y, Vhat, par)
...
if nargin<8, par = []; end                         % analyse parameter structure
...
Korig = feval(cov{:}, hyp.cov, x);                  % evaluate the covariance matrix
K = Korig +  diag(Vhat);                               %WW edited
