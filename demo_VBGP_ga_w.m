...
k = 25;                 % number of design points
C = 25;               % total computation budget
...
x_tst = x_tst./(ones( size(x_tst,1) ,1)*exp(lengthscales(:)'));
    ...
    LambdaTheta0 = [log(0.5)*ones(k,1);loghyperSignal;loghyperNoise;mu0]; % D + 2 + 2 + 1
...
f = @(LambdaTheta)vhgpr_w_ga_derivfree(LambdaTheta, covfuncSignal, covfuncNoise, X, y, A);
%LambdaTheta = ga(f,length(LambdaTheta)) 

LambdaTheta = fminunc(f,LambdaTheta0)
%[out1]= vhgpr_w_ga_derivfree(LambdaTheta, covfunc1, covfunc2, X, y, A)
...
x_tst = x_tst./(ones( size(x_tst,1) ,1)*exp(lengthscales(:)'));
    ...
    display('Initializing VHGPR (keeping hyperparameters fixed)...')
    LambdaTheta = LambdaTheta2; [log(0.5)*ones(k,1);loghyperSignal;loghyperNoise;mu0]; % D + 2 + 2 + 1
[LambdaTheta, convergence0] = minimize(LambdaTheta, 'vhgpr_w_1101', 30, covfuncSignal, covfuncNoise, 2, x_tr, y_tr, A); % A added % error here % 2=fixhyp
